...
- **Bodies carry code, not prose.** No comment of any syntax inside a function body, and none parked just outside it either — on the struct field, the `const`, the `match` arm. Name for the reader six months from now and let the name do the work. Nor is the rule satisfied by moving the prose somewhere it still ships: an `.expect("…")` string, an assertion message, a `tracing::` call, or an identifier stretched into a sentence are the same narration at a higher price.
- **One exception: a claim the code cannot make about itself, asserted where the risk is.** Why this `.unwrap()` cannot fail, why this `unsafe` is sound, why this lint is suppressed, why formatting is off here. In Rust that is usually the justification on `.unwrap()`/`.expect()`, a `SAFETY:` block, or the reason on `#[allow(…)]`, `#[expect(…)]` or `#[rustfmt::skip]` — but the criterion governs, not that list, so a saturating fallback whose harmlessness no nearby line establishes qualifies too. Write the claim, not the label: what makes this infallible, not that it is. And the test is strict — if the reader can derive it from the code, it is derivable, and it goes.
- **The why moves out of drift range** — to the doc comment on the item, the commit message, the PR description, or the decision log for L work. Pick by audience, not convenience: a doc comment on a public item is published to callers, so the alternative you weighed and refused goes in the commit or the PR, never there. On a private item the doc comment absorbs it fine. The cost is real and worth naming: `git blame` is the index from a line to its reason, and a refactor breaks that index quietly, where a stale comment at least announces itself. Reasoning that belongs to a *line* is the signal that the line wants to be an item; extracting a one-line helper purely to host a sentence is not, and "Depth beats count" already refuses it. Where what needs explaining is a literal or an operator order rather than a name — a bit mask, a rounding constant, a fixed offset — bind it to a named `const` or a named local. That is naming, and unlike a comment it moves with the value.
- **Doc comments stay, and one that paraphrases its own identifier documents nothing.** `/// Time an empty bucket needs to refill completely` over `fn time_to_full()` buys a restatement and costs a line that can go stale on its own. Where the name already carries the meaning, add what it cannot — the unit, what makes it panic, the precondition a caller has no way to see, what an algorithm is called when the code alone would send a reader hunting for the paper — or leave the item bare. Where a lint enforces coverage, the bare branch is closed and the item gets the sentence its name could not carry. It is not a relocation site for line-by-line narration: it says what the item is and what a caller must know, never the steps inside it. And a precondition worth documenting is often one that should have been encoded instead: see "Before writing code" before settling for prose.
