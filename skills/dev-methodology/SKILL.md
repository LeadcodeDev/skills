...
- **Name for the reader six months from now, and let the name do the work.** What is left for a comment is what the code cannot say: why this way rather than the obvious way, the constraint that forced it, what an algorithm is called and where it comes from when the code alone would send a reader hunting for the paper, the issue carrying the rest. The check is mechanical — cover the comment and read the code, and if a reader learns the same thing, it was never documentation.
- **The check does not reach a comment another rule or the toolchain requires**: the justification on an `.unwrap()`, a `SAFETY:` block, the reason on an `#[allow(…)]`. Those restate their surroundings deliberately, because the claim has to be asserted where the risk is rather than reconstructed from two lines further up. Write them, and write them as the claim — what makes this infallible, not that it is.
- **A doc comment that paraphrases its own identifier documents nothing.** `/// Time an empty bucket needs to refill completely` over `fn time_to_full()` buys a restatement and costs a line that can go stale on its own. Where the name already carries the meaning, add what it cannot — the unit, what makes it panic, the precondition a caller has no way to see — or leave the item bare. A convention that every public item be documented is about coverage, not a licence to pad; where a lint enforces it, the bare branch is closed and the item gets the sentence its name could not carry. And a precondition worth documenting is often one that should have been encoded instead — see "Before writing code" before settling for prose.
