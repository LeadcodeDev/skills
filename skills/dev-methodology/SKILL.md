...
**How messages to the user are written lives elsewhere.** The `to-the-point` file — French and `tu`, the first line carrying the answer, no preamble and no closer, the banned-phrase list, the pre-send check, two options and a recommendation on a decision handed over — is imported into every session from the user's `CLAUDE.md` and is the canonical copy. It is not repeated here, so that the two cannot drift. Read it there if this document is all you were handed and its rules are not already in your context.

What stays here is what is specific to development work.

**Artifacts go the other way on language.** Commit messages, issues, and PRs are written in English whatever language the conversation is in (see "Git essentials").

**Say it once, then stop, applies to artifacts too.** Volume is easiest to add to a plan, a spec, a mini-spec or a review remark, because there it looks like rigour. A sentence that could be deleted without the reader losing a fact or an instruction is a sentence to delete, and that holds for every output this document shapes, not for chat alone.
