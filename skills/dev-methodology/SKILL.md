...
- **Under a page, there are no headings.** `## Context`, `## Why this matters`, `## Scope` above five paragraphs is a table of contents for a text nobody will navigate. Headings earn their place once a reader has to come back and find one part again; below that, write the paragraphs in the order the reader needs them and let the first sentence of each carry what the heading would have said.
- **State the problem; do not plead the case.** A reader who would object has not objected yet, and a paragraph written against that objection doubles the body to answer nobody. Say what is broken, say what to build, stop. One line survives: the one that pre-empts a refusal the change genuinely invites, such as a specification that looks like it forbids it.
